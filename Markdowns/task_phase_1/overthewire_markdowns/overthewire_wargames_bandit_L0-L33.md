...
- i learnt about a strings command and then used to for the file which essentially finds the readable text and then used pipe command followed by grep argumented with equal signs since the password is preceded by multiple == signs and hence finding this provides the password

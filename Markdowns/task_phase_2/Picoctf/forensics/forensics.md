...
- now i opened the debian package and found various files there and once i go into one of them i saw steghide and then i searched up steghide and it opened a page and understood that it is a method of hiding data in audio/video so then on terminal i used man command to understand the arguments required for steghide
